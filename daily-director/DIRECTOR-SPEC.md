...
**The section order is enforced by `sb.js`, not by the page**, so it can never drift. Top to bottom: the finished film (when it exists), `סינופסיס`, `שוט-ליסט`, `קריינות ודיאלוג`, `אווירה והלך רוח`, `סיפור רקע`, `הערות הפקה`, and last the **פרומפט נושא** block. Every section is collapsible with a chevron on the side. Defaults: `שוט-ליסט` and `קריינות ודיאלוג` start OPEN, everything else starts CLOSED. The open/closed state is remembered per page in localStorage.

- **סינופסיס** is MANDATORY on every board, no exceptions. `sb.js` always renders the section, and a page without a `synopsis` block shows a visible "חסר סינופסיס" warning instead of hiding it, so one can never ship missing. It is the film told as prose in Eyal's voice, 5-7 short paragraphs, what actually happens beat by beat, ending on the thesis line. Add `en` for the English version under 300 words; the template renders it as a collapsible block with its own copy button, because competition cover sheets ask for exactly that.
- **שוט-ליסט** is the shot cards: generated frames, per-shot duration, camera, model pill, and copy-prompt buttons. Prefer TWO prompts per shot, one for the opening frame and one for the motion, returned by the page's `shots.prompts(shot, ratio)` function. When a shot also has a rendered clip, give it `vid`; the template then shows a **תמונות / וידאו** switch at the top of the section and swaps every frame between the still and the player at once. Stills are the default view. When a film has more than one delivery ratio, add `shots.ratios` and give each shot per-ratio `img` and prompt fields; the switch then swaps frames and prompts together.
- **פרומפט נושא** is a single button at the bottom of every page that copies the entire board as ONE English prompt, hierarchically ordered: logline, format, synopsis, visual style, base style string, shot list with both prompts per shot, dialogue, backstory, production notes, and a fixed production-guidance block. It is generated by `sb.js` from the page data, never written by hand. To keep it usable by an English model, give every Hebrew block an English twin: `titleEn`, `specsEn`, `mood.en`, `story.en`, `notes.itemsEn`, `shots.style`, and per shot `titleEn` / `camEn` / `durEn`. Where a twin is missing the template falls back to the Hebrew, which works but reads worse.
...
Reference implementations: `storyboards/future-of-learning/` (clips per shot, full English twins, master prompt) and `storyboards/skill-vs-prompt/` (the ratio switch). All published boards run on this template. Daily runs do NOT create storyboards; only an explicit "פתח תסריט" request does.

## Design language (locked)
The main app page carries the SAME sticky app bar as the storyboard pages, one to one (round back button on the right pointing to `../`, the daily director wordmark centred with the green underline, paper background with blur and a hairline bottom border). The tab bar sticks directly beneath it at `top:63px`. If the app bar changes in `storyboards/shared/sb.css`, mirror it in `index.html` in the same commit.
