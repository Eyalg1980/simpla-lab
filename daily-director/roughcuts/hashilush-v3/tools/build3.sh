#!/bin/bash
# usage: bash build3.sh "<presigned put url>"
set -e
UP="$1"
...
echo "== audio, vo placed at the marks build3.py computed"
FC=""
IN=""
LBL=""
i=0
...
echo "== mux"
ffmpeg -nostdin -y -loglevel error -i silent.mp4 -i track.m4a -c:v copy -c:a copy -shortest hashilush-v3-roughcut.mp4 </dev/null
ffprobe -v error -show_entries format=duration,size -show_entries stream=codec_name,width,height -of default=nw=1 hashilush-v3-roughcut.mp4

echo "== upload"
curl -f -s -X PUT -H "Content-Type: video/mp4" --upload-file hashilush-v3-roughcut.mp4 "$UP" && echo UPLOAD_OK
